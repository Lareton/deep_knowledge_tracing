...
def performance(ground_truth, prediction):
    ...
    # попробовать сохранить fpr tpr, ground_truth, predictions

    with open("metrics.pickle", "wb") as f:
        pickle.dump([ground_truth.detach().cpu().numpy(), prediction.detach().cpu().numpy()], f)
...
class lossFunc(nn.Module):
    ...
    def forward_old(self, pred, batch):
        loss = 0
...
def train_epoch(model, trainLoader, optimizer, loss_func, device, epoch_number=0):
    ...
    torch.save(model, f"model_{epoch_number}.weights")
